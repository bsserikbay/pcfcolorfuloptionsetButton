...
export class ColorfulOptionSetButton implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	/**
	...
	public init(context: ComponentFramework.Context<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement) {
		...
		// Get all options
		this.optionSetArray = context.parameters.OptionSetAttribute.attributes?.Options;
		// UI
		// Main Container
		...
		if (this.optionSetArray) {
			...
			for (let i = 0; i < this.optionSetArray.length; i++) {
				// Button
		...
		this.theContainer.appendChild(this.eleMainContainer);
	}
	/**
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void {
		if (this.eleMainContainer.children.length > 0) {
			...
			for (var i = 0; i < this.eleMainContainer.children.length; i++) {
				let elementButton = this.eleMainContainer.children[i] as HTMLButtonElement;
